...
import com.example.assignment.exception.EntityException;
import com.example.assignment.util.ConnectionHelper;
import com.example.assignment.util.ConvertHelper;
import com.example.assignment.util.SQLConstant;
...
public class JpaRepository<T> {
    ...
    // constructor nhận tham số đầu vào là một kiểu class, thông tin này
    ...
    private List<T> fullFillObject(ResultSet resultSet) throws InstantiationException, IllegalAccessException, SQLException {
        ...
        while (resultSet.next()) { //hàm next() để chỏ xuống dòng tiếp theo, false thì dừng, true chạy tiếp
            //resultSet có thể lấy dl kiểu resultSet.getString("...")
